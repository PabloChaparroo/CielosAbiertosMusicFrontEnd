...
import { EmptyState, FavButton } from "@/components/common/ui-bits";
import { useApp } from "@/hooks/useApp";
import { chordsOnly, KEYS, parseChordPro, transposeKey, type ParsedLine } from "@/lib/chords";
      ...
      <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {filtered.length === 0 ? (
              ...
              filtered.map((s) => (
                ...
                >
                  <div className="h-8 w-8 shrink-0 rounded-lg" style={{ backgroundImage: s.cover }} />
        ...
        <div className="order-1 space-y-5 lg:order-2">
          <div className="surface-card flex flex-wrap items-center gap-3 p-4">
            ...
            <div className="flex items-center gap-1 rounded-full border border-border p-1">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const semis =
                    (((KEYS.indexOf(target) - idxFrom) % 12) + 12) % 12;
            ...
            <div className="flex items-center gap-1 rounded-full border border-border p-1">
              <IconBtn onClick={() => setFontSize((f) => Math.max(12, f - 2))} label="Achicar letra">
                <Minus className="h-4 w-4" />
              </IconBtn>
              <span className="w-10 text-center text-xs text-muted-foreground">{fontSize}px</span>
              <IconBtn onClick={() => setFontSize((f) => Math.min(40, f + 2))} label="Agrandar letra">
                <Plus className="h-4 w-4" />
              </IconBtn>
            </div>
...
function ChordSheet({
  ...
  return (
    <div
      className={`font-mono leading-tight ${centered ? "text-white" : ""}`}
      style={{ fontSize }}
    >
      {lines.map((line, i) => {
...
function Annotations({ songId }: { songId: string }) {
  const {
    annotations,
    addAnnotation,
    updateAnnotation,
